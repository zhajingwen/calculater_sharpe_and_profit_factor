...
def print_metric_row(label: str, value: str, unit: str = "", color: str = "") -> None:
    ...
    if color:
        value_display = f"{color}{Style.BRIGHT}{value:>{value_width}}{Style.RESET_ALL}"
    else:
        value_display = f"{Fore.WHITE}{value:>{value_width}}{Style.RESET_ALL}"

    unit_display = f" {Fore.LIGHTBLACK_EX}{unit}{Style.RESET_ALL}" if unit else ""

    print(f"  {Fore.CYAN}•{Style.RESET_ALL} {Fore.WHITE}{label:<{label_width}}{Style.RESET_ALL} {value_display}{unit_display}")
...
def print_separator(char: str = "─", width: int = 80) -> None:
    ...
    Args:
        ...
        width: 宽度
    """
    print(f"  {Fore.LIGHTBLACK_EX}{char * (width - 2)}{Style.RESET_ALL}")
...
def display_header() -> None:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}║{Style.RESET_ALL}  {Fore.LIGHTBLACK_EX}基于 Hyperliquid 官方 API 和 Apex Liquid Bot 算法{Style.RESET_ALL}{'  ' * 5}{Fore.CYAN}{Style.BRIGHT}║{Style.RESET_ALL}")
...
def display_core_metrics(analysis: AnalysisResults) -> None:
    ...
    if sharpe_on_trades and sharpe_on_trades.get('total_trades', 0) > 0:
        print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ Sharpe Ratio (风险调整收益) {'─' * 36}┐{Style.RESET_ALL}")
        ...
        if sharpe_val > 1:
            ...
            rating = f"{Fore.YELLOW}⚠ 偏高风险{Style.RESET_ALL}"
        ...
        print(f"  {Fore.YELLOW}{Style.BRIGHT}└{'─' * 76}┘{Style.RESET_ALL}")
        print(f"  {Fore.LIGHTBLACK_EX}  💡 单笔收益率 = closedPnL / (|sz| × px){Style.RESET_ALL}")

    # Max Drawdown 已移除
    # 原因：基于PNL的回撤计算不够准确，无法反映真实的资金风险

    # 交易统计
    print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 交易统计 {'─' * 62}┐{Style.RESET_ALL}")
...
def display_account_info(analysis: AnalysisResults) -> None:
    ...
    print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 账户价值 {'─' * 62}┐{Style.RESET_ALL}")
    print_metric_row('总账户价值', f"${total_account_value:,.2f}", "", Fore.GREEN + Style.BRIGHT)
    print(f"  {Fore.CYAN}  ├─{Style.RESET_ALL} {Fore.LIGHTBLACK_EX}Perp 账户{Style.RESET_ALL}          {Fore.CYAN}${perp_account_value:>18,.2f}{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}  └─{Style.RESET_ALL} {Fore.LIGHTBLACK_EX}Spot 账户{Style.RESET_ALL}          {Fore.CYAN}${spot_account_value:>18,.2f}{Style.RESET_ALL}")
    print_separator()
    ...
    print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 盈亏统计 {pnl_icon} {'─' * 60}┐{Style.RESET_ALL}")
    print_metric_row('累计总盈亏', f"${total_cumulative_pnl:,.2f}", "", pnl_color + Style.BRIGHT)
    print(f"  {Fore.CYAN}  ├─{Style.RESET_ALL} {Fore.LIGHTBLACK_EX}已实现盈亏{Style.RESET_ALL}        {real_color}${total_realized_pnl:>18,.2f}{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}  └─{Style.RESET_ALL} {Fore.LIGHTBLACK_EX}未实现盈亏{Style.RESET_ALL}        {unreal_color}${total_unrealized_pnl:>18,.2f}{Style.RESET_ALL}")
    print(f"  {Fore.YELLOW}{Style.BRIGHT}└{'─' * 76}┘{Style.RESET_ALL}")
    ...
    if has_valid_roe:
        ...
        print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 多周期 ROE {roe_icon} {'─' * 58}┐{Style.RESET_ALL}")
        ...
        for roe_data, label in [
            ...
            if roe_data.get('is_valid', False):
                ...
                print(f"  {Fore.CYAN}•{Style.RESET_ALL} {Fore.WHITE}{label:<28}{Style.RESET_ALL} {Fore.RED}✗ {error_msg[:20]}{Style.RESET_ALL}")
        ...
        if warnings:
            print(f"  {Fore.YELLOW}  ⚠ 注意:{Style.RESET_ALL} " + ", ".join(warnings))
            print(f"  {Fore.LIGHTBLACK_EX}  ROE 基于实际时长计算{Style.RESET_ALL}")
        ...
        try:
            ...
            print(f"  {Fore.LIGHTBLACK_EX}  🕐 更新时间: {end_time_str}{Style.RESET_ALL}")
        except:
            pass

    else:
        # 显示错误信息
        print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 多周期 ROE {'─' * 60}┐{Style.RESET_ALL}")
    ...
    print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 收益率指标 {'─' * 60}┐{Style.RESET_ALL}")
    ...
    print(f"  {Fore.YELLOW}{Style.BRIGHT}└{'─' * 76}┘{Style.RESET_ALL}")
    print(f"  {Fore.LIGHTBLACK_EX}  💡 基于单笔交易持仓价值计算，不依赖外部本金{Style.RESET_ALL}")
...
def display_hold_time_stats(analysis: AnalysisResults) -> None:
    ...
    print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 平均持仓时长 {'─' * 58}┐{Style.RESET_ALL}")
...
def analyze_user_trading(user_address: str, force_refresh: bool = False,
    ...
    try:
        ...
        print(f"{Fore.CYAN}{Style.BRIGHT}│{Style.RESET_ALL}  {Fore.YELLOW}{Style.BRIGHT}📊 分析用户{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{Style.BRIGHT}│{Style.RESET_ALL}  {Fore.MAGENTA}{user_address}{Style.RESET_ALL}")
        ...
        if "error" in results:
            ...
            print(f"\n  {Fore.RED}{Style.BRIGHT}┌─ ✗ 错误 {'─' * 66}┐{Style.RESET_ALL}")
            print(f"  {Fore.CYAN}•{Style.RESET_ALL} {Fore.RED}分析失败: {results['error']}{Style.RESET_ALL}")
            print(f"  {Fore.RED}{Style.BRIGHT}└{'─' * 76}┘{Style.RESET_ALL}")
            return False
        ...
        if generate_report:
            ...
            print(f"\n  {Fore.YELLOW}{Style.BRIGHT}┌─ 报告生成 {'─' * 62}┐{Style.RESET_ALL}")
        ...
        print(f"\n\n  {Fore.YELLOW}{Style.BRIGHT}┌─ ⚠ 警告 {'─' * 66}┐{Style.RESET_ALL}")
...
def display_help() -> None:
    ...
    print(f"  {Fore.CYAN}{Style.BRIGHT}┌─ 用法 {'─' * 68}┐{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}•{Style.RESET_ALL} {Fore.WHITE}python main.py [用户地址] [选项]{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}{Style.BRIGHT}└{'─' * 76}┘{Style.RESET_ALL}\n")

    print(f"  {Fore.YELLOW}{Style.BRIGHT}┌─ 参数 {'─' * 68}┐{Style.RESET_ALL}")
    ...
    print(f"  {Fore.YELLOW}{Style.BRIGHT}┌─ 选项 {'─' * 68}┐{Style.RESET_ALL}")
    ...
    print(f"  {Fore.CYAN}{Style.BRIGHT}┌─ 示例 {'─' * 68}┐{Style.RESET_ALL}")
    ...
    print(f"  {Fore.GREEN}{Style.BRIGHT}┌─ 功能说明 {'─' * 62}┐{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}•{Style.RESET_ALL} {Fore.GREEN}✓ 交易级别指标（推荐）{Style.RESET_ALL} {Fore.LIGHTBLACK_EX}- 完全不受出入金影响{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}•{Style.RESET_ALL} {Fore.YELLOW}⚠ 账户级别指标{Style.RESET_ALL} {Fore.LIGHTBLACK_EX}- 受出入金影响，仅供对比参考{Style.RESET_ALL}")
    print(f"  {Fore.GREEN}{Style.BRIGHT}└{'─' * 76}┘{Style.RESET_ALL}\n")
